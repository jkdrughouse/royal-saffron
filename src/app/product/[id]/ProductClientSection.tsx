...
import { ShoppingCart, ArrowLeft, Star, CheckCircle2, Heart } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { products, type Product, type ProductVariant } from "@/app/lib/products";
        ...
        <div className="container mx-auto px-4 py-8 sm:py-12 md:py-16">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
                ...
                {product.images && product.images.length > 0 ? (
                    ...
                    <div className="relative aspect-square bg-muted/10 rounded-lg p-6 sm:p-8">
                        <Image
                            src={product.image}
                            alt={product.name}
                            fill
                            ...
                            className="object-contain mix-blend-multiply p-6 sm:p-8"
                        />
                    </div>
                )}
                ...
                <div className="flex flex-col">
                        ...
                        <div className="flex items-baseline gap-3">
                            ...
                            {currentOriginalPrice && currentOriginalPrice > currentPrice && (
                                <>
                                    ...
                                    <span className="text-sm font-semibold text-primary">Save ₹{currentOriginalPrice - currentPrice}</span>
                                </>
                            )}
                        </div>
                    ...
                    {product.variants && product.variants.length > 0 && (
                        <div className="mb-6 sm:mb-8">
                            <label className="block font-serif text-lg sm:text-xl mb-3">Weight (Gms)</label>
                            <select
                    ...
                    {product.sensoryDescription && (
                        <div className="mb-6 p-4 bg-gradient-to-br from-[#FFF8F0] to-white rounded-lg border border-saffron-crimson/20">
                            <p className="text-sm text-muted-foreground leading-relaxed">{product.sensoryDescription}</p>
                        </div>
                    )}
                    ...
                    <div className="mt-auto pt-6 border-t flex gap-3">
                        <Button onClick={handleAddToCart} size="lg" className="flex-1 gap-2 text-lg px-8 py-7 min-h-[56px]">
                        ...
                        <nav className="flex gap-4 sm:gap-8 -mb-px">
                            {(['description', 'additional', 'reviews'] as const).map((tab) => (
                                <button
                                    ...
                                    className={`pb-4 px-2 sm:px-4 font-medium text-sm sm:text-base transition-colors border-b-2 capitalize ${activeTab === tab ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                                >
                                    {tab === 'reviews' ? `Reviews (${reviewCount})` : tab === 'additional' ? 'Additional Information' : 'Description'}
                                </button>
                            ))}
                        </nav>
                    </div>
                                                                ...
                                                                if (trimmed.match(/^[🌼🍯🌿☕💋🌸🛡️🚫🥝💚😋✨🍒☀️🥜🧈💪🌰🧠🌱🌹🍫🎁🧼•]/)) {
                                                                    return (
                                                                        <p key={pIdx} className="flex items-start gap-2">
                                                                            ...
                                                                            <span>{trimmed.replace(/^[🌼🍯🌿☕💋🌸🛡️🚫🥝💚😋✨🍒☀️🥜🧈💪🌰🧠🌱🌹🍫🎁🧼•]\s*/, '')}</span>
                                                                        </p>
                                                                    );
                                                                }
                            ...
                            <div className="space-y-4">
                                <table className="w-full border-collapse">
                                    <tbody className="divide-y divide-muted">
                                        {product.weightMl && (
                                            <tr>
                                                <th className="text-left py-3 px-4 font-semibold text-foreground border-r border-muted w-1/3">Weight</th>
                                                <td className="py-3 px-4 text-muted-foreground">{product.weightMl}</td>
                                            </tr>
                                        )}
                                        {product.sku && (
                                            <tr>
                                                <th className="text-left py-3 px-4 font-semibold text-foreground border-r border-muted w-1/3">SKU</th>
                                                <td className="py-3 px-4 text-muted-foreground">{product.sku}</td>
                                            </tr>
                                        )}
                                        <tr>
                                            <th className="text-left py-3 px-4 font-semibold text-foreground border-r border-muted w-1/3">Category</th>
                            ...
                            .map((relatedProduct) => (
                                <Link key={relatedProduct.id} href={`/product/${relatedProduct.id}`}>
                                    <Card className="flex flex-col h-full border-muted/20 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                                        <div className="relative aspect-square p-4 bg-muted/10 rounded-t-lg">
                                            <Image
